...
왼쪽턴10:
    ...
    MOVE G6A,  97,  83, 145,  90,  98,
    MOVE G6D,  97,  63, 145, 110,  98,
    WAIT

    SPEED 12
    MOVE G6A,  94,  83, 145,  88,  99,
    MOVE G6D,  94,  63, 145, 108,  99,
...
오른쪽턴10:
    ...
    SPEED 8
    'MOVE G6A,97,  63, 145,  108, 103, 100
    'MOVE G6D,97,  83, 145,  88, 103, 100

    MOVE G6A,  97,  63, 145, 110, 100,
    MOVE G6D,  97,  83, 145,  90, 100,
    WAIT

    SPEED 12
    MOVE G6A,94,  63, 145,  108, 101, 100
    MOVE G6D,94,  83, 145,  88, 101, 100
    WAIT
    SPEED 6
    MOVE G6A,101,  76, 146,  93, 98, 100
    MOVE G6D,101,  76, 146,  93, 98, 100
...
MAIN_2:
    ...
    ZERO G6B,106, 102, 100, 100, 100, 102
    ZERO G6C,101,  99,  97, 100, 100, 100
...
aaaa:
    ...
    IF A > 158 AND A < 253 THEN
        GOSUB m왼쪽턴20
    ELSEIF 0 < A AND A < 98 THEN
        GOSUB m오른쪽턴20
    ELSEIF A >= 138  AND A <= 158 THEN
        GOSUB 왼쪽턴10
...
장애물Y_G:
    SERVO 16, 38
    넘어진확인 = 0
    ETX 4800, 122       'green center y 보내기
    장애물YG = 0
    ff=0
ygyg:
    ff= ff + 1
...
caca:
    ...
    IF A >= 0 AND A < 135 THEN
        GOSUB 양발한번앞으로가기1
        GOSUB 양발한번앞으로가기2
        GOSUB 기울기보정_G1
    ELSEIF A >= 135 AND A <= 255 THEN
        RETURN
...
dada:
    ...
    IF A >= 0 AND A < 155 THEN
        GOSUB 양발한번앞으로가기1
        GOSUB 양발한번앞으로가기2
        GOSUB 기울기보정
    ELSEIF A >= 155 AND A <= 255 THEN
        RETURN
...
보정종합_G:
    ...
    GOSUB 장애물거리_G1
    GOSUB 기울기보정_G1
    GOSUB 좌우보정_G2                   '고개 내리고, 세밀하게 좌우보정
    GOSUB 장애물거리_G2                 '세밀하게 앞으로 가기
    RETURN
